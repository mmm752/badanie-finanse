...
def save_data_multi_sheet(data_package):
    ...
    # Nazwy arkuszy w GSheets (POZOSTAJĄ BEZ ZMIAN)
    SHEET_NAMES = {
    ...
    try:
        ...
        if can_use_gsheets:
            try:
                ...
                # --- ZMIANA NAZWY PLIKU GOOGLE SHEETS TUTAJ ---
                sh = gc.open("Wyniki_Badania_Internet")
                # ----------------------------------------------
    ...
    with st.form(f"survey_form_likert_{page_num}"):
        for q_data in current_questions:
            ...
            val_str = st.select_slider(
                ...
                value="3 – Trudno powiedzieć / Neutralnie",
                key=q_data['id'],
